...
public class MetodosSql extends Thread {
    ...
    //    System.out.println("Mostrando Query --->"+SentenciaSql+"<---");
        ...
        try {
            ...
            con.conectar(this.server,this.database,this.usuario,this.password);
            if(con!= null) {
            txtrLogs.append("Conectado!\n");
            con.resulsete=con.statemente.executeQuery(SentenciaSql);
            txtrLogs.append("Consultando "+SentenciaSql+"\n");
            res = con.resulsete;
        ...
        if(arreglo.isEmpty()) {
        	txtrLogs.append("No hay resultados para la consulta\n");
        	return "";
        	
        }else {
        	txtrLogs.append("Generando resultados\n");
        	txtrLogs.append(arreglo.get(0));
    ...
    public void run(String sentencia, JTextArea txtrLogs) {
        try {
			llenarJtable(sentencia, txtrLogs);
    ...
    public int insertarOmodif(String sentenciaSql,JTextArea txtrLogs) {
        ...
        try {
        	txtrLogs.append("Intentando actualizar");
        	con.conectar(this.server,this.database,this.usuario,this.password);
            con.statemente.executeUpdate(sentenciaSql);
 
            con.desconectar();
            status=1;
            txtrLogs.append("Actualizado Correcto");
    ...
    public JTable llenarJtable(String sentencia, JTextArea txtrLogs ) throws InterruptedException{
        Conexion con = new Conexion();
        ...
        txtrLogs.append("Cargando datos...\n");
        Thread.sleep(1500);
          ...
          txtrLogs.append("Entregando Datos.\n");
          Thread.sleep(1500);
         
          Thread.sleep(1500);
          txtrLogs.append("Fin cargar Tabla\n");
